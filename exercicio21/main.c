/*25-A cidade de São Paulo tem uma lei que restringe a circulação de carros numa determinada área conhecida como "Centro Expandido". Com base no número final da placa, o motorista sabe se deve ou não circular no horário proibido. Faça um programa que receba um número inteiro correspondente aos dígitos da placa do automóvel e verifique se é o dia de rodízio ou não. Considere:
...
int main(){
	...
	int n;
	printf("Digite os números da placa de seu carro ");
	scanf("%d",&n);
	n=n%10;
